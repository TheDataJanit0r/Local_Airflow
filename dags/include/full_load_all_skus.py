def run_full_load():
    ...
    for df_chunk in df_product:
        ...
        if count == 0:
            ...
            chunk.head(0).to_sql(pg_tables_to_use, pg_engine,schema=pg_schema, if_exists='replace',index=False) #drops old table and creates new empty table
            print("Success")
        ...
        writer.writerows(chunk.replace('"','').values)
        sio.seek(0)

        with Postgres_cursor as c:
            c.copy_from(
                file=sio,
                table=os.getenv('PG_ALL_SKUS'),
